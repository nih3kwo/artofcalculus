...
:::{aside} 
```{seealso} Схема сыра
1. Смотрим на последовательности левых концов отрезков $(a_n)$ и правых концов $(b_n)$. Показываем, что они монотонны и ограничены из вложенности отрезков.
2. Из теоремы Вейерштрасса существуют пределы.
3. Показать равенство пределов из предела длины отрезка.
4. Общий предел и есть общая точка.
```
:::
...
```{aside} 
:::{seealso} Схема сыра
**ТУДА** (из сходимости в фундаментальность (очев))

Оценить $|x_n - x_m|$ через определение предела.

**СЮДА** (из фундаментальности в сходимость (не очев))

1. Взять произвольную положительную бесконечно малую последовательность $\varepsilon_k$.
2. Построить отрезки вида $J_k = [x_{N_k} - \varepsilon_k, x_{N_k} + \varepsilon_k]$, пользуясь фундаментальностью.
3. Показать, что $J_k \cap J_{k+1}$ не пусто.
4. Построить отрезки $J_k = J_1 \cap J_2 \cap \ldots \cap J_k$. 
5. Показать, что система отрезков $J_k$ удовлетворяет лемме о вложенных отрезках: вложенность, предел длины.
6. По лемме о вложенных отрезках получить общую точку $J_k$, которая и есть предел из произвольности $(\varepsilon_k)$.
:::
```
...
$$
длины которых стремятся к нулю. Тогда по [лемме о вложенных отрезках](#cap_of_intervals) существует хотя бы одна $a \in \cap_{k=1}^\infty I_k$. Но тогда $a \in [a_{N_k}-\varepsilon_k, a_{N_k}+\varepsilon_k]$ для любого $k$, а это значит, что для любого $\varepsilon_k$ мы всегда знаем такой номер $N_k$, что $|a_n - a|<\varepsilon_k <2 \varepsilon_k$. Последнее в силу единственности предела и произвольной последовательности из $\varepsilon_k$ показывает, что $\lim_{n\to \infty}a_n = a$.
:::
...
:::{aside}
```{warning}
О метрических пространствах будет подробнее рассказано позже [в этой главе](#metric-space-definition). Не пугайтесь, если вы чего-то до конца не понимаете, эта часть дана скорее как лирическое отступление.
```
:::
