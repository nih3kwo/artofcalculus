...
Оценить $abs(x_n - x_m)$ через определение предела.
...
4. Построить отрезки $I_k = I_1 \cap I_2 \cap \ldots \cap I_k$. 
5. Показать, что система отрезков $I_k$ удовлетворяет лемме о вложенных отрезках: вложенность, предел длины.
6. По лемме о вложенных отрезках получить общую точку $I_k$, которая и есть предел из произвольности $(\varepsilon_k)$.
:::
```
