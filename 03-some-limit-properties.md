...
Докажем, что $\lim_{n\to \infty}(a_nb_n - ab) = 0$, тогда по [этому предложению](#lim(a_n-a)=0) будет следовать требуемое.
...
Так как $\lim_{n\to \infty} b_n =b>0$, то по [теореме об отделимости](#separate) найдётся такой $N_0$, что для всех $n>N_0$ будет иметь место неравенство $b_n > \frac{b}{2}>0$. 
