...
:::{prf:proof}
:class: dropdown
:nonumber: Мы воспользуемся леммой [](#almost_for_series) в случае необходимосити и тогда можем считать, что неравенства выполнены для всех $n \ge 0.$
...
(3) Пусть $q<1$. Возьмём такой $\varepsilon>0$, чтобы $q<q+\varepsilon <1$. Тогда по определению предела [](#limit_of_seqeunce) найдётся такой $N$, что при всех $n \ge N$ мы получаем
$$
