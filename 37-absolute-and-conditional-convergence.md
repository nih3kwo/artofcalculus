...
Наконец, так как ряд $(x_n)$ сходится, то по необходимому признаку (см. следствие [](#required-condition-of-convergence)), $\lim_{n \to \infty} x_n = 0$, а из того, что $(x_n^+)$, $(x_n^-)$ подпоследовательности в последовательности $(x_n)$, то из теоремы [](#lim(sub)=lim) получаем, что $\lim_{n \to \infty} x_n^+ = \lim_{n \to \infty} x_n^- = 0.$ Это завершает доказательство.
:::
...
:class: dropdown
:nonumber:
Для ряда $(x_n)$ мы рассмотрим ряды $(x_n^+)$, $(x_n^-)$ (см. конструкцию [](#x_nnn)). Согласно предложению [](#conditional_convergence_and_x_n), ряды $(x_n^+)$, $(x_n^-)$ расходятся. Это значит, что последовательности их частичных сумм неограничены (см. теорема [](#criteria_for_positive_series)), **т.е.** их значения могут быть больше любого числа. 
...
Далее, согласно предложению [](#conditional_convergence_and_x_n), $\lim_{k \to \infty}x_{p_k+1}^+ = 0$, то по лемме о зажатой последовательности (см. лемма [](#squeezy)) и предложению [](#lim(a_n-a)=0) получаем, что
...
А тогда, согласно предложению [](#conditional_convergence_and_x_n), $\lim_{k \to \infty}x_{p_k+1}^+ = 0$, то по лемме о зажатой последовательности (см. лемма [](#squeezy)) и предложению [](#lim(a_n-a)=0) получаем, что
