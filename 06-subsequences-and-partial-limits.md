...
1. Доказать лемму, что если $A subset.eq B$ оба ограничены, то $sup(A) <= sup(B)$ и $inf(A) >= inf(B)$.
...
5. Пользуясь тем, что $M_k -$ супремумы, построить подпоследовательность $(a_{n_k}) \subseteq (a_n)$ так, чтобы $M_{n_{k - 1}} - 2^{1 - k} <= a_{n_k} <= M_{n_{k-1}}$. 
