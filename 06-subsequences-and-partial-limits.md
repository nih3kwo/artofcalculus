...
(5) Тогда по построению мы получили для любого $n_k$, $|x_{n_k}-c| < |I_k| = \frac{I_1}{2^{k}}$. Но это и означает, что $\lim_{n_k \to \infty}x_{n_k} = c.$
:::
...
1. По определению супремума и инфинума оценить $a_{n_k}$.
2. Воспользоваться леммой о переходу в пределах в неравенстве.
:::
```
