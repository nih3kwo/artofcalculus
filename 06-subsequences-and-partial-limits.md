...
1. Рассмотрим последовательность $(a_n)=\{1,2,3,1,2,3,\ldots,\}$, тогда если положить, что $n_k = 2k$ для всех $k\ge 1$, то мы получаем подпоследовательность $a_{n_k} = \{2,1,3,2,1,3, \ldots,\}$. Но если мы выберем номера так, чтобы $n_1= 2, n_2 = 1, n_3=1, \ldots,$ то мы получим последовательность $2,1,1,\ldots,$ которая уже подпоследовательностью не является, так как $n_1 = 2 <n_2 = 1.$
...
1. Доказать лемму, что если $A \subseteq B$ оба ограничены, то $\sup(A) \leq \sup(B)$ и $\inf(A) \geq \inf(B)$.
2. Раз $M_1 \supseteq M_2 \supseteq M_3 \dots$, то по лемме последовательность $(M_k)$ не возрастает. Аналогично, $(m_k)$ не убывает.
3. Раз $(a_n)$ ограничена, то $(m_k)$ и $(M_k)$ тоже.
4. Последовательности ограничены и монотонны, а значит, есть пределы.
5. Пользуясь тем, что $M_k -$ супремумы, построить подпоследовательность $(a_{n_k}) \subseteq (a_n)$ так, чтобы $M_{n_{k - 1}} - 2^{1 - k} \leq a_{n_k} \leq M_{n_{k-1}}$. 
6. По лемме о двух миллиционерах у $(a_{n_k})$ нужный предел. Аналогично для $(m_k)$.

**ОГРАНИЧЕННОСТЬ ЧАСТИЧНЫХ ПРЕДЕЛОВ**
1. По определению супремума и инфимума оценить $a_{n_k}$.
2. Воспользоваться леммой о переходе в пределах в неравенстве.
:::
```
...
:class: dropdown
:nonumber:
Во-первых, заметим, что существование точной грани в зависимости от характера ограниченности обеспечивается [принципом полноты Вейерштрасса](#W=complete). 
...
$$
то по [лемме](#AleB=sup(A)lesup(B)) $M_n \ge M_{n+1}$, т. е. последовательность $ (M_n )$ не возрастает и также $m_n \le m_{n+1}$, т. е. последовательность $ (m_n )$ не убывает.
...
Ясно, что $\left(M_{n_k} \right)$ есть подпоследовательность последовательности $\left(M_n\right)$, но мы уже знаем, что $\lim_{n\to \infty}M_n = M$, тогда по [теореме](#lim(sub)=lim), $\lim_{n_k\to \infty}M_{n_k} = M$. Наконец, по [лемме о зажатой последовательности](#squeezy)получаем, что $\lim_{n_k \to \infty}a_{n_k} = M.$
